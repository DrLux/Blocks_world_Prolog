...
% goal_position(A) = true if the block sequence from A to pillar is the same in S and G
goal_position(X,S,G):-
...
replace_block(X,Y,S,SP):-
	...
	replace_onpillar(X,Y,FAKE_S,SP).
